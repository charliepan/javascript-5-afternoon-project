...
//It should take in three parameters: id (a number), title (a string) and rating (a number). Use these parameters to create a new object and add it to 
//the savedPosts array. Make sure to name the properties the same as described previously (id, title, rating).
...
User.prototype.removeSavedPost = function(id){
  ...
  for(let i  = 0; i < this.savedPosts.length; i++){
    index = this.savedPosts[i].id === id ? i:0;
  }
...
User.prototype.changePostRating = function(id, rating){
  let index = this.savedPosts.map((val)=> val.id).indexOf(id);
